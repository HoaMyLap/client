...
export default function PricingPage() {
  ...
  return (
    <div className="min-h-screen w-full bg-background text-foreground relative overflow-x-hidden font-sans pb-20">
      ...
      {/* Pricing Cards Grid */}
      <main className="max-w-5xl mx-auto px-6 grid grid-cols-1 md:grid-cols-3 gap-8 relative z-10 mt-10 items-stretch">
        {plans.map((plan, idx) => (
          <div 
            key={idx} 
            className={`glass p-8 rounded-3xl border flex flex-col justify-between relative transition-all duration-300 hover:scale-[1.01] ${
              plan.popular 
          ...
          >
            {plan.popular && (
              <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-gradient-to-r from-primary to-accent text-white text-[10px] font-black uppercase tracking-wider px-3.5 py-1 rounded-full shadow-md">
                Phổ biến nhất
              </div>
            )}
            ...
            <div>
              <div className="flex justify-between items-start mb-4">
                <h3 className="text-xl font-bold font-display text-heading">{plan.name}</h3>
              </div>
              ...
              <div className="mb-8 flex items-baseline gap-1">
                {typeof plan.price === 'number' ? (
                  <>
                    ...
                    <span className="text-muted text-xs">/thành viên/tháng</span>
                  </>
                ) : (
            ...
            <Link 
              href={plan.href}
              className={`w-full py-3.5 rounded-2xl text-xs font-bold text-center block transition-all ${
                plan.popular 
      ...
      <section className="max-w-md mx-auto text-center mt-16 px-6 relative z-10 flex items-center justify-center gap-2 text-xs text-muted">
